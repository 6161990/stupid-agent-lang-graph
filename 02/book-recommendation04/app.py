...
async def run_graph(inputs):
    ...
    step = 0
    total_steps = 3  # search_book_titles + write_book_letter 두 단계
    ...
    try:
        async for output in graph.astream(inputs, stream_mode="updates"):
            for key, value in output.items():
                ...
                elif key == "detect_category":
                    detected_category = value.get("category", "알 수 없음")
                    category_status.success(f"✓ 카테고리 판별 완료: {detected_category}")
